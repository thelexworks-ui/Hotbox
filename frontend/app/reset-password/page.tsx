...
function PasswordStrength({ password }: { password: string }) {
  const score = password.length === 0 ? 0 : password.length < 6 ? 1 : password.length < 10 ? 2 : /[A-Z]/.test(password) && /[0-9]/.test(password) ? 4 : 3;
...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="glass-card rounded-2xl p-8 text-center">
        ...
        <p className="text-[rgba(232,244,248,0.50)] text-sm mb-6">This password reset link is invalid or has expired.</p>
        <a href="/forgot-password" className="text-[#5ADAEE] text-sm hover:text-[#3BBDD1] transition-colors">
          Request a new link
        </a>
      </div>
    );
  ...
  return (
    <div className="glass-card rounded-2xl p-8">
      ...
      <div className="w-12 h-12 rounded-xl bg-[rgba(90,218,238,0.08)] border border-[rgba(90,218,238,0.18)] flex items-center justify-center mb-7">
      ...
      <form className="space-y-4" autoComplete="on" onSubmit={handleSubmit}>
        <div>
          <label className="block text-[rgba(232,244,248,0.60)] text-xs font-mono uppercase tracking-widest mb-2">New password</label>
          <div className="relative">
            <input type={showNew ? 'text' : 'password'} name="new-password" autoComplete="new-password"
        ...
        <div>
          <label className="block text-[rgba(232,244,248,0.60)] text-xs font-mono uppercase tracking-widest mb-2">Confirm new password</label>
          <div className="relative">
            <input type={showConf ? 'text' : 'password'} name="confirm-password" autoComplete="new-password"
          ...
          {reqs.map((req) => (
            <div key={req.label} className="flex items-center gap-2">
              <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 transition-all duration-200 ${req.met ? 'bg-[#5ADAEE]' : 'bg-[rgba(232,244,248,0.08)] border border-[rgba(232,244,248,0.15)]'}`}>
                {req.met && <svg width="8" height="6" viewBox="0 0 8 6" fill="none"><path d="M1 3L2.8 5L7 1" stroke="#050C14" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
              </div>
              <span className={`text-xs transition-colors duration-200 ${req.met ? 'text-[rgba(232,244,248,0.60)]' : 'text-[rgba(232,244,248,0.30)]'}`}>{req.label}</span>
            </div>
          ))}
        </div>
